...
#not in use
def collect(sym, start= '', end = '',  interval = Client.KLINE_INTERVAL_1DAY):
    #row in csv, NEWER entries below
    Open_time = []
    Open= []
    High= []
    ...
    collect = True
    latest_close_time = 0
    result = getHistoryPrice(sym, "16 hours ago UTC")
    for row in result:
    ...
    for i in range(0, 10):
        result = getHistoryPriceByRange(sym,str(end_close_time), str(latest_close_time))
    ...
    writeToCSV(history_df, str(ts) + '_to_' + str(latest_close_time))
